...
function _hreflang_set (req, seo) {
    ...
    // TODO: Controllare che originalUrl al posto di route.path non rompa qualcosa
    const current_path = req.originalUrl.replace(/\/$/, '');
...
function _seo_middleware (req, res, next) {
    ...
    if (options.open_graph || options.twitter_cards) {
